app.controller("personal",function($scope, $http, $timeout){
	$scope.proyectos = [];
	$scope.listado_personal= [];
		...
		$http.post(lnk, {}).then(
			function(resp){
				if(resp.data.status){
					...
					$scope.listado_personal = resp.data.listado;
				}else{
					console.log(resp.data)
				}
			},
...
app.controller("import_personal",function($scope, $http, $timeout){
