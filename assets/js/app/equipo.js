app.controller("equipo",function($scope, $http, $timeout){	
	$scope.proyectos = [];
	$scope.listado_equipos= [];
		...
		$http.post(lnk, {}).then(
			function(resp){
				if(resp.data.status){
					...
					$scope.listado_equipos = resp.data.listado;
				}else{
					console.log(resp.data)
				}
			},
...
app.controller("import_equipo",function($scope, $http, $timeout){
